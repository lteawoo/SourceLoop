...
The system SHALL allow operators to constrain planning to a selected subset of supported question families, regardless of whether the planner is sourcing questions from the built-in template set or an AI-authored batch.
...
The system SHALL render enough planner metadata in the question batch archive to explain why the batch contains its current shape, including when the batch was sourced from AI-authored question drafts.
